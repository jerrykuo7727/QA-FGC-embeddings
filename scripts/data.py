...
class BertDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        with open('data/%s/datedur_mask/%s' % (self.split, '|'.join(question_id.split('|')[:2]))) as f:
            datedur_mask = [int(binary) for binary in f.read().split(' ')]
        with open('data/%s/num_mask/%s' % (self.split, '|'.join(question_id.split('|')[:2]))) as f:
            num_mask = [int(binary) for binary in f.read().split(' ')]
        ...
        if diff > 0:
            if self.split == 'train':
                if answer_end > 510:
                    ...
                    datedur_mask = datedur_mask[diff:]
                    num_mask = num_mask[diff:]
                    answer_start -= diff
                    answer_end -= diff
                else:
                    passage = passage[:-diff]
        ...
        datedur_mask.append(0)
        num_mask.append(0)
        input_tokens = question + passage
        input_tokens_no_unk = question_no_unk + passage_no_unk
        datedur_mask = [0 for _ in question] + datedur_mask
        num_mask = [0 for _ in question] + num_mask
        ...
        num_mask = torch.LongTensor(num_mask)
        if self.split == 'train':
            start_positions = torch.LongTensor([answer_start]).squeeze(0)
            end_positions = torch.LongTensor([answer_end]).squeeze(0)
            return input_ids, attention_mask, token_type_ids, start_positions, end_positions, datedur_mask, num_mask
        else:
            margin_mask = torch.FloatTensor([*(-1e10 for _ in question), *(0. for _ in passage[:-1]), -1e-10])
            return input_ids, attention_mask, token_type_ids, margin_mask, input_tokens_no_unk, answer, datedur_mask, num_mask
...
def get_dataloader(model_type, split, tokenizer, bwd=False, batch_size=1, num_workers=0, prefix=None):
    ...
    def train_collate_fn(batch):
        input_ids, attention_mask, token_type_ids, start_positions, end_positions, datedur_mask, num_mask = zip(*batch)
        ...
        datedur_mask = pad_sequence(datedur_mask, batch_first=True, padding_value=0)
        num_mask = pad_sequence(num_mask, batch_first=True, padding_value=0)
        return input_ids, attention_mask, token_type_ids, start_positions, end_positions, datedur_mask, num_mask
    
    def test_collate_fn(batch):
        input_ids, attention_mask, token_type_ids, margin_mask, input_tokens_no_unk, answers, datedur_mask, num_mask = zip(*batch)
        input_ids = pad_sequence(input_ids, batch_first=True)
        attention_mask = pad_sequence(attention_mask, batch_first=True)
        token_type_ids = pad_sequence(token_type_ids, batch_first=True, padding_value=1)
        ...
        datedur_mask = pad_sequence(datedur_mask, batch_first=True, padding_value=0)
        num_mask = pad_sequence(num_mask, batch_first=True, padding_value=0)
        return input_ids, attention_mask, token_type_ids, margin_mask, input_tokens_no_unk, answers, datedur_mask, num_mask
